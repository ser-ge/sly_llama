"""
Using sly and sly_llama to implement MRLKL
"""
...
class MrklLexer(Lexer):
    ...
    #ignore ne lines so that we can match multiline tokens
    ignore = "\t \n"
    ...
    def ACTION_INPUT(self, token):
        token.value = (
                token.value.replace("Action Input:", "")
        )
        return token
    ...
    def FINAL_ANSWER(self, token):
        token.value = (
            token.value.replace("Final Answer", "").strip("\n ")
        )
...
@llm_call(llm, stop_sequence="Observation", verbose=False, return_prompt=True, return_llm_output=True)
...
def mrlkl_agent(query: str, tools_list: List[Tool], max_iters: int, max_retries: int) -> str | None:
    """
    ...
    for _ in range(max_iters):
        ...
        if mrlkl_output.action in tools:
            ...
            current_observation = f'{mrkl_start.action} not a valid tool, try another one'

        for i in range(max_retries):

            try:
                mrlkl_output, last_prompt, raw_output = mrkl_step(history, current_observation)
                break
